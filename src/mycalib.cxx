...
/*
ChangLog:
  -- Query replace dataRead.getConfig -> config.get
*/
...
int main ( int argc, char **argv ) {
   ...
   while ( dataRead.next(&event) ) {
      ...
      // Get injection times
      if ( eventCount == 0 ) {
         minDac        = config.getInt("CalDacMin");
         minChan       = config.getInt("CalChanMin");
         maxChan       = config.getInt("CalChanMax");
         injectTime[0] = config.getInt("cntrlFpga:kpixAsic:Cal0Delay");
         injectTime[1] = config.getInt("cntrlFpga:kpixAsic:Cal1Delay") + injectTime[0] + 4;
         injectTime[2] = config.getInt("cntrlFpga:kpixAsic:Cal2Delay") + injectTime[1] + 4;
         injectTime[3] = config.getInt("cntrlFpga:kpixAsic:Cal3Delay") + injectTime[2] + 4;
         injectTime[4] = 8192;
      }
   ...
   positive    = (config.get("cntrlFpga:kpixAsic:CntrlPolarity") == "Positive");
   b0CalibHigh = (config.get("cntrlFpga:kpixAsic:CntrlCalibHigh") == "True");
