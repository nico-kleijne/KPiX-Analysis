...
#def readdir(directory):
	...
	#for key in directory.GetListOfKeys():
			...
			###print key.GetName()
			#directory.cd(key.GetName())
			#subdir = gDirectory
			#readdir(subdir)
			#print gDirectory
			#print dirsav
			#print "TEST ", key.GetName()
	#dirsav.cd()
...
#def search_through(directory, count, dir_name):
	#Found = False
	##print directory
	#for key in directory.GetListOfKeys():
		#if ('TDirectory' in key.GetClassName()):
				...
				#directory = gDirectory.Get(dir_name)
			#else:
				#dir_name = dir_name + '/' + key.GetName()
			...
			#count = count + 1
			##print count
			#final = search_through(directory, count, dir_name)
	#if (Found == False):
...
def map_plot(channel_kpix, mapping, sensor_row, sensor_column, color, list_of_channel_values, bucket_name, bucket_nr, color_values, type_name, color_type):
		fig = plt.figure()
...
def get_data(key, adcmean_kpix, adchannel_kpix, adcRMS_kpix, num_of_entries_kpix, total):
	if (('TH1F' in key.GetClassName()) and ('hist' in key.GetName()) and (args.kpix in key.GetName())  and ('time' not in key.GetName()) and ('_b' in key.GetName())):  #Take only TH1 histograms; ; Take only the normal histograms not the channel summaries
		hist = key.ReadObj()
...
parser.add_argument('file_in', help='an integer for the accumulator')
parser.add_argument('style', help='an integer for the accumulator')                    
...
#parser.add_argument('-s', '--style', dest='style', help='Type of plot, 2D = 2Dwith colors, 3D = 3D')
...
if (not args.style):
	print "Please specify the type of plot with -s (2D/3D)"
	quit()
...
if ('2D' in args.style):
	...
	#for b in bucket:  #mean map
		#map_plot(channel_kpix, mapping_sensor_to_kpix, row, column, color, adcmean_sensor, b, bucket_nr, color, 'mean', 'log')
		#bucket_nr = bucket_nr + 1
		
#	color_RMS = [0, 0.002, 0.004, 0.006, 0.010, 0.015, 0.020, 0.1]
#	bucket_nr = 0
#	for b in bucket: #rms map
#		map_plot(channel_kpix, mapping_sensor_to_kpix, row, column, color_RMS, adcRMS_sensor, b, bucket_nr, color_RMS, 'rms', 'log')
#		bucket_nr = bucket_nr + 1
		
	color_entry = [0.0, 1E0] 
	bucket_nr = 0
	for b in bucket: #entries
		map_plot(channel_kpix, mapping_sensor_to_kpix, row, column, color_entry, num_of_entries_sensor, b, bucket_nr, color_entry, 'entry', 'norm')
		bucket_nr = bucket_nr + 1
	...
	#		map_plot(channel_kpix, mapping_sensor_to_kpix, row, column, color_slope, channel_slope_sensor, b, bucket_nr, color_slope, 'slope', 'norm')
	#		bucket_nr = bucket_nr + 1
			
elif ('3D' in args.style):	
	bucket_nr = 0
	for b in bucket: 
		plot_3D(channel_kpix, row, column, adcmean_sensor, b, bucket_nr, 'mean')
		bucket_nr = bucket_nr + 1
	bucket_nr = 0
	#for b in bucket:
	#	plot_3D(channel_kpix, row, column, adcRMS_sensor, b, bucket_nr, 'rms')
	#	bucket_nr = bucket_nr + 1
	#bucket_nr = 0
	#for b in bucket:
	#	plot_3D(channel_kpix, row, column, channel_slope_sensor, b, bucket_nr, 'slope')
	#	bucket_nr = bucket_nr + 1
else:
	print 'Please choose a valid plotting style, either 2D or 3D'
	quit()
